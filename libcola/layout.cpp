...
static void apply_routing_with_libavoid(
    Context &ctx,
    const json &c,
    json &edges_out,
    double &routing_time_ms,
    int &total_turns,
    std::string &routing_mode_out
) {
    auto start = std::chrono::steady_clock::now();

    std::string routing = c.value("routing", "none");
    routing_mode_out = routing;
    total_turns = 0;

    if (routing == "none") {
        edges_out = json::array();
        for (auto &e : ctx.input["edges"]) {
            json je;
            je["source"] = e["source"];
            je["target"] = e["target"];
            edges_out.push_back(je);
        }
        auto end = std::chrono::steady_clock::now();
        routing_time_ms = std::chrono::duration<double, std::milli>(end - start).count();
        return;
    }
    ...
    for (size_t k = 0; k < ctx.input["edges"].size(); ++k) {
        ...
        // number of turns = number of segments - 1
        if (pl.size() >= 2) {
            total_turns += static_cast<int>(pl.size()) - 1;
        }
...
int main(int argc, char **argv) {
    ...
    bool no_echo_constraints = false;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--no-echo-constraints") {
            no_echo_constraints = true;
    ...
    while (read_request(std::cin, buf, input_separator)) {
        ...
        layout_time_ms = std::chrono::duration<double, std::milli>(layout_end - layout_start).count();
        ...
        apply_routing_with_libavoid(ctx, c, edges_out, routing_time_ms, total_turns, routing_mode);

        // Engine stats
        int node_count = static_cast<int>(nodes_out.size());
        int edge_count = static_cast<int>(ctx.input["edges"].size());

        json engine_info = json::object();
        engine_info["layout_time_ms"]  = layout_time_ms;
        engine_info["routing_time_ms"] = routing_time_ms;
        engine_info["node_count"]      = node_count;
        engine_info["edge_count"]      = edge_count;
        engine_info["total_turns"]     = total_turns;
        engine_info["routing_mode"]    = routing_mode;
        engine_info["warnings"]        = json::array();
        engine_info["engine_version"]  = "1.0";

        // Build metadata
        json metadata_out;
        bool has_input_metadata = ctx.input.contains("metadata");
        if (!no_metadata) {
            if (has_input_metadata) {
                metadata_out = ctx.input["metadata"];
            } else {
                metadata_out = json::object();
            }
            metadata_out["engine"] = engine_info;
        }

        // Build output with preserved key order, metadata last
        json out = json::object();

        std::vector<std::string> key_order;
        key_order.reserve(ctx.input.size());
        for (auto it = ctx.input.begin(); it != ctx.input.end(); ++it) {
            const std::string key = it.key();
            if (key == "command") continue; // internal
            key_order.push_back(key);
        }

        for (const auto &key : key_order) {
            if (key == "metadata") {
                // handled later, always last
                continue;
            } else if (key == "id") {
                out["id"] = ctx.input["id"];
            } else if (key == "constraints") {
                if (!no_echo_constraints && ctx.input.contains("constraints")) {
                    out["constraints"] = ctx.input["constraints"];
                }
            } else if (key == "nodes") {
                out["nodes"] = nodes_out;
            } else if (key == "edges") {
                ...
                // echo any other top-level keys as-is
                out[key] = ctx.input[key];
            }
        }
