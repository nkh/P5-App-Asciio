...
// If you have libavoid available, include its headers here.
...
static void build_nodes_and_edges(Context &ctx) {
    ...
    for (size_t i = 0; i < input["nodes"].size(); i++) {
        ...
        // Rectangle is defined by minX, maxX, minY, maxY
...
// Placeholder for libavoid routing integration.
// You would construct a Router, add shapes for nodes, and route edges.
// For now, we just leave edges without routes.
static void apply_routing(Context &ctx, const json &c, json &out) {
    std::string routing = c.value("routing", "none");
    if (routing == "none") {
        // No routing, just copy edges without route.
    ...
    // Here is where libavoid would be used. For now, we emit a trivial straight-line route.
    out["edges"] = json::array();
    for (auto &e : ctx.input["edges"]) {
        ...
        auto *sr = ctx.rects[si];
        auto *tr = ctx.rects[ti];

        double sx = sr->getCentreX();
        double sy = sr->getCentreY();
        double tx = tr->getCentreX();
        double ty = tr->getCentreY();

        json je;
        je["source"] = sid;
        je["target"] = tid;
        je["route"] = json::array();
        je["route"].push_back(json::array({sx, sy}));
        je["route"].push_back(json::array({tx, ty}));
...
int main(int argc, char **argv) {
    ...
    while (read_request(std::cin, buf, input_separator)) {
        ...
        try {
            ...
            // Invalid JSON, skip or report error.
            continue;
        }
