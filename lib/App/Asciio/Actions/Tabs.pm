...
sub open_project           { my ($self) = @_                 ; $self->signal_emit('open_project', '', 1)                                 ;                           }
sub next_tab               { my ($self) = @_                 ; $self->signal_emit('next_tab')                                            ;                           }
sub next_tagged_tab        { my ($self, $tag) = @_           ; $self->signal_emit('next_tagged_tab', $tag)                               ;                           }
sub previous_tab           { my ($self) = @_                 ; $self->signal_emit('previous_tab')                                        ;                           }
sub previous_tagged_tab    { my ($self, $tag) = @_           ; $self->signal_emit('previous_tagged_tab', $tag)                           ;                           }
...
sub get_asciios            { my ($self) = @_                 ; $self->signal_emit('get_all_asciios')                                     ;                           }
sub rename_asciio_tab      { my ($self, $asciio, $name) = @_ ; $self->signal_emit('rename_asciio_tab', $asciio, $name)                   ;                           }
