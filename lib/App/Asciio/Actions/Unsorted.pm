...
my $codes = $self->display_edit_dialog('Enter codes to display', '', $self) ;

if(defined $codes && $codes ne '')
	{
	my @lines = split("\n", $codes);
	my @conver_lines ;
	my $line_num = @lines;
	my $max_width = length("$line_num") ;
	for my $line_index (0 .. ($line_num - 1))
		{
		push @conver_lines, ' ' x ($max_width - length("$line_index")) . $line_index . '|' . $lines[$line_index] ;
		}
