...
use utf8 ;
package App::Asciio::stripes::triangle_up ;
...
elsif($x == $self->{WIDTH} - int($self->{HEIGHT} / 2) && $y == $middle_height && ($self->{HEIGHT} % 2 == 1))
	{
	return {X =>  $x, Y => $y, NAME => 'right_center'} ;
	}
elsif($x == $self->{WIDTH} - int($self->{HEIGHT} / 2) + 1 && $y == $middle_height && ($self->{HEIGHT} % 2 == 0))
	{
	return {X =>  $x, Y => $y, NAME => 'right_center'} ;
	}
...
return
	...
	{X =>  $right_x, Y => $middle_height, NAME => 'right_center'},
	) ;
}
