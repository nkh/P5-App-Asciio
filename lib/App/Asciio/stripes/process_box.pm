...
for my $line (@lines)
	{
	push @lines_width_plus_offset, unicode_display_width($line) + abs($current_half_the_lines) ;
...
for my $line (@top_lines)
	...
	my $padding = ' ' x ($text_width_plus_offset  - (unicode_display_width($line) + $current_half_the_lines)) ;
...
my $padding = ' ' x ($text_width_plus_offset  - unicode_display_width($center_line)) ;
$strip_text = ') ' . $center_line . $padding . ' )' ;
$element_width =  unicode_display_width($strip_text) + $y_offset - 1 ; # first stripe is two lines high, compensate offset by substracting one
