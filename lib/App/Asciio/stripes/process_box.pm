...
for my $line (@lines)
	{
	push @lines_width_plus_offset, get_unicode_length($line) + abs($current_half_the_lines) ;
...
for my $line (@top_lines)
	...
	my $padding = ' ' x ($text_width_plus_offset  - (get_unicode_length($line) + $current_half_the_lines)) ;
...
my $padding = ' ' x ($text_width_plus_offset  - get_unicode_length($center_line)) ;
$strip_text = ') ' . $center_line . $padding . ' )' ;
$element_width =  get_unicode_length($strip_text) + $y_offset - 1 ; # first stripe is two lines high, compensate offset by substracting one
